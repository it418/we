...
// API base: make it work even when deployed under a sub-path (e.g. /portal/universe/)
// Example: https://host/portal/universe/index.html  ->  /portal/universe/api
// Robust base path (works for /universe, /universe/, /universe/index.html, /portal/universe, etc.)
const BASE_DIR = (() => {
  const p = location.pathname || '/';
  if (p.endsWith('/')) return p;
  const last = p.split('/').pop() || '';
  // If the last segment looks like a file (has a dot), strip filename
  if (last.includes('.')) return p.slice(0, p.lastIndexOf('/') + 1);
  // Otherwise treat it as a directory path without trailing slash
  return p + '/';
})();
const API = BASE_DIR.replace(/\/+$/, '') + '/api';
...
async function postJson(path,body){try{const r=await fetch(API+path,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body||{})});const j=await safeJson(r);return r.ok?j:Object.assign({success:false,message:j.message||j.error||"Error"},j)}catch(e){return{success:false,message:"Network: "+e.message}}}
async function getJson(path,params){try{const u=new URL(API+path,location.origin);if(params)Object.entries(params).forEach(([k,v])=>u.searchParams.set(k,v));const r=await fetch(u);const j=await safeJson(r);return r.ok?j:Object.assign({success:false,message:j.message||j.error||"Error"},j)}catch(e){return{success:false,message:"Network: "+e.message}}}
async function postForm(path,fd){try{const r=await fetch(API+path,{method:"POST",body:fd});const j=await safeJson(r);return r.ok?j:Object.assign({success:false,message:j.message||j.error||"Error"},j)}catch(e){return{success:false,message:"Network: "+e.message}}}
...
function exportData(kind){if(kind==="requests")window.open(`${API}/export/requests.csv?actor=${encodeURIComponent(me.username)}&ts=`+Date.now(),"_blank");if(kind==="inventory")window.open(`${API}/export/inventory.csv?actor=${encodeURIComponent(me.username)}&ts=`+Date.now(),"_blank")}
